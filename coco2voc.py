...
def coco_to_voc_detection(coco_annotation_file, target_folder,pesudo_folder):
    ...
    for index, image_id in enumerate(coco_instance.imgToAnns):
        ...
        pesudo_path =os.path.join(pesudo_folder,file_name+'.json')
        if os.path.exists(pesudo_path):
            pesudo_objs = json.load(open(pesudo_path,'r'))
            for unk_obj in pesudo_objs:
                bbox = unk_obj['bbox']
                score = unk_obj['score']
                unk_object_el = ET.SubElement(annotation_el,'unk_object')
                ET.SubElement(unk_object_el,'name').text = 'unk'
                ET.SubElement(unk_object_el,'difficult').text = '0'
                ET.SubElement(unk_object_el,'score').text = str(score)
                bb_el = ET.SubElement(unk_object_el,'bndbox')
                ET.SubElement(bb_el, 'xmin').text = str(int(bbox[0] + 1.0))
                ET.SubElement(bb_el, 'ymin').text = str(int(bbox[1] + 1.0))
                ET.SubElement(bb_el, 'xmax').text = str(int(bbox[2] + 1.0))
                ET.SubElement(bb_el, 'ymax').text = str(int(bbox[3] + 1.0))
        else:
            print(f'{pesudo_path} is not exists')
            with open("data/OWDETR/VOC2007/ImageSets/Main/only_unknow.txt", "a") as myfile:
