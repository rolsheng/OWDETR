...
def parse_args():
    ...
    parser.add_argument('--sam_predict',type=str,default="sam_predict",help="path to folder of sam predict")
...
def load_glip_predict(path):
    obj_file = json.load(open(path,'r'))
    glip_instances = [obj['bbox'] for obj in obj_file]
    glip_scores = [obj['score'] for obj in obj_file]
...
def load_sam_predict(path,seg_path,overlap = 0.3):
    ...
    for obj in obj_file:
        ...
        for ids in BackGround_ids:
            if region.count(ids)/area > overlap:
                flag = False
                break
        if flag:
