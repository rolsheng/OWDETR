...
class SetCriterion(nn.Module):
    ...
    def __init__(self, args, num_classes, matcher, weight_dict, losses, invalid_cls_logits, focal_alpha=0.25):
        ...
        self.enable_adaptive_pseudo = args.enable_adaptive_pseudo
        self.pseudo_store_path = args.pseudo_store_path
        self.adaptive_update_iter = args.adaptive_update_Iter
    ...
    def forward(self, samples, outputs, targets, epoch,Iter, last_loss):
        ...
        if self.unmatched_boxes and epoch >= loss_epoch:
            ...
            for i in range(len(indices)):
                ...
                if 'unk_boxes' in targets[i]:
                    unk_boxes = box_ops.box_cxcywh_to_xyxy(deepcopy(targets[i]['unk_boxes']))
                    unk_boxes = unk_boxes*torch.tensor([img_w,img_h,img_w,img_h],dtype=torch.float32).to(owod_device)
                else :
                    unk_boxes = torch.tensor([])
